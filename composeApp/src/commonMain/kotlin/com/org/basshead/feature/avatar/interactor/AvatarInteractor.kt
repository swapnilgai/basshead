...
import com.org.basshead.feature.dashboard.interactor.AvatarsKey
...
class AvatarInteractorImpl(
    ...
    override suspend fun getAvatars(): List<Avatar> = withInteractorContext(
        cacheOption = CacheOptions(
            key = AvatarsKey(),
            expirationPolicy = longCacheExpiration, // 30 minutes, avatars change infrequently
        ),
    ) {
        supabaseClient.from("avatars")
            .select()
            .decodeList<Avatar>()
    }
